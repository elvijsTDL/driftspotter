      ...
      >
        ...
        <div className="p-6">
          ...
          <p className="text-sm text-muted mb-4">
            Organized by{" "}
            {event.submittedBy ? (
              <a href={`/organizers/${event.submittedBy}`} className="text-foreground font-medium hover:text-drift-orange transition-colors underline decoration-border underline-offset-4">
                {event.organizer}
              </a>
            ) : (
                      ...
                      {missingEmergencyContact && (
                        <div className="flex items-start gap-2 rounded-xl bg-amber-500/10 border border-amber-500/30 px-3 py-2.5">
                          ...
                          <p className="text-xs text-amber-200/90 leading-snug">
                            ...
                            <a href="/profile" className="underline font-semibold hover:text-amber-100">profile</a>{" "}
                            before applying.
                          </p>
                        </div>
                      )}
                      <p className="text-xs text-muted-dark">
                        Your <a href="/profile" className="text-drift-cyan hover:underline">driver profile</a> (car, photos, event history) is shared with the organizer — keep it up to date.
                      </p>
              ...
              {attendees.length > 0 && (
                <div className="flex -space-x-2">
                  {attendees.slice(0, 5).map((a) => (
                    <a key={a.user_id} href={`/drivers/${a.user_id}`} title={a.username} className="hover:z-10 hover:-translate-y-0.5 transition-transform">
                      {a.avatar_url ? (
